...
const useClapAnimation = ({
    ...
    useLayoutEffect(() => {
        ...
        // Para que funcione si tiene refs o un string con el id del elemento
...
const Clap = () => {
    ...
    const [{ clapRef, countRef, clapCountTotalRef }, setRefState] = useState({})
    ...
    const handlerClickButtonClap = () => {
        ...
        setClapState(prevState => ({
            ...
            countTotal:
                count < MAXIMUM__USER__CLAPS ? prevState.countTotal + 1 : prevState.countTotal
        }))
    }
    return (
        <button
...
const ClapCount = ({ count, setRef }) => {
    // id="count"
    return <span ref={setRef} data-refkey="countRef" className={styles.count}>{`+${count}`}</span>
}

const ClapTotal = ({ countTotal, setRef }) => {
    // id="clapCountTotal"
    return <span ref={setRef} data-refkey="clapCountTotalRef" className={styles.total}>{`${countTotal}`}</span>
}
